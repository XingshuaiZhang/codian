...
An Obsidian plugin that embeds Codex CLI as a sidebar chat interface. The vault directory becomes Codex's working directory, giving it full agentic capabilities: file read/write, bash commands, and multi-step workflows.
...
| **utils** | Utility functions | date, path, env, editor, session, markdown, diff, context, sdkSession, frontmatter, slashCommand, mcp, codexCliResolver, externalContext, externalContextScanner, fileLink, imageEmbed, inlineEdit |
...
| File | Contents |
|------|----------|
| `.codex/obsidian/runtime-settings.json` | Runtime-compatible permissions and plugin-enabled runtime state |
| `.codex/obsidian/settings.json` | Plugin settings (model, UI, environment, navigation, etc.) |
| `.codex/obsidian/mcp.json` | MCP server configs |
| `.codex/obsidian/commands/*.md` | Slash commands (YAML frontmatter) |
| `.codex/agents/*.md` | Custom agents (YAML frontmatter) |
| `.agents/skills/*/SKILL.md` | Skill definitions |
| `.codex/obsidian/sessions/*.jsonl` | Plugin-owned conversation transcripts |
| `.codex/obsidian/sessions/*.meta.json` | Session metadata |
| `~/.codex/sessions/{vault}/*.jsonl` | Runtime-native session messages |

## Development Notes

- **SDK-first**: Proactively use native Codex CLI features over custom implementations. If the SDK provides a capability, use it — do not reinvent it. This ensures compatibility with Codex CLI.
- **SDK exploration**: When developing SDK-related features, write a throwaway test script (e.g., in `dev/`) that calls the real SDK to observe actual response shapes, event sequences, and edge cases. Real output lands in `~/.codex/` or `{vault}/.codex/` — inspect those files to understand patterns and formats. Run this before writing implementation or tests — real output beats guessing at types and formats. This is the default first step for any SDK integration work.
