...
import type { CanUseTool, RuntimeOptions } from '../types/sdk';
...
export interface QueryOptionsContext {
  ...
  /** Plugin manager for Codex CLI plugins. */
...
export class QueryOptionsBuilder {
  ...
  static needsRestart(
    ...
    if (currentConfig.codexCliPath !== newConfig.codexCliPath) return true;
  ...
  static buildPersistentQueryConfig(
    ...
    return {
      ...
      codexCliPath: ctx.cliPath,
      enableChrome: ctx.settings.enableChrome,
    };
  }

  /** Builds SDK options for the persistent query. */
  static buildPersistentQueryOptions(ctx: PersistentQueryContext): RuntimeOptions {
    ...
    const options: RuntimeOptions = {
      ...
      pathToCliExecutable: ctx.cliPath,
      settingSources: ['project'],
      env: {
        ...process.env,
        ...ctx.customEnv,
    ...
    if (ctx.resume) {
      ...
      if (ctx.resume.sessionAt) {
        ...
        options.resumeMessageId = ctx.resume.sessionAt;
      }
      if (ctx.resume.fork) {
    ...
    options.spawnCliProcess = createCustomSpawnFunction(ctx.enhancedPath);

    return options;
  }

  /** Builds SDK options for a cold-start query. */
  static buildColdStartQueryOptions(ctx: ColdStartQueryContext): RuntimeOptions {
    const permissionMode = ctx.settings.permissionMode;
  ...
  private static applyExtraArgs(options: RuntimeOptions, settings: CodianSettings): void {
