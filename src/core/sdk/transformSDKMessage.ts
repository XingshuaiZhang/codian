...
import type { SDKMessage, SDKResultError } from '../types/sdk';
...
function getBuiltInModelSignature(model: string): { family: 'gpt-5-codex' | 'gpt-5' | 'o4-mini' } | null {
  const normalized = model.trim().toLowerCase();
  if (normalized === 'gpt-5-codex') {
    return { family: 'gpt-5-codex' };
  }
  if (normalized === 'gpt-5') {
    return { family: 'gpt-5' };
  }
  if (normalized === 'o4-mini') {
    return { family: 'o4-mini' };
  }
  return null;
}

function getModelUsageSignature(model: string): { family: 'gpt-5-codex' | 'gpt-5' | 'o4-mini' } | null {
  const normalized = model.trim().toLowerCase();
...
function selectContextWindowEntry(
  ...
  const signatureMatches = entries.filter((entry) => {
    ...
    return entrySignature?.family === intendedSignature.family;
  });
