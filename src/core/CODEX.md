...
|--------|---------|-----------|
| `agent/` | Codex CLI runtime wrapper | `CodianService` (incl. fork session tracking), `SessionManager`, `QueryOptionsBuilder` (incl. `resumeSessionAt`), `MessageChannel`, `customSpawn` |
...
| `plugins/` | Codex CLI plugins | `PluginManager` |
...
const service = new CodianService(plugin, vaultPath);
...
### Storage (Codex CLI pattern)
```typescript
// Settings in vault/.codex/settings.json
await CCSettingsStorage.load(vaultPath);
await CCSettingsStorage.save(vaultPath, settings);

// Sessions: runtime-native (~/.codex/projects/) + metadata overlay (.meta.json)
...
- `CodianService` must be disposed on tab close (abort + cleanup)
- `SessionManager` handles SDK session resume via `sessionId`
- Fork uses `pendingForkSession` + `pendingResumeAt` on `CodianService` to pass `resumeSessionAt` to SDK; these are one-shot flags consumed on the next query
