 ...
 *
 * Manages:
 * - Runtime settings in .codian/obsidian/runtime-settings.json
 * - Plugin settings in .codian/obsidian/settings.json
 * - Slash commands in .codian/obsidian/commands/*.md
 * - Chat sessions in .codian/obsidian/sessions/*.jsonl
 * - MCP configs in .codian/obsidian/mcp.json
 *
 * Handles migration from legacy formats:
 * - Old settings.json with plugin fields → split into runtime + plugin files
 * - Old permissions array → CC permissions object
 * - data.json state → settings.json
 */
...
/** Base path for plugin-owned storage. */
export const OBSIDIAN_CODIAN_PATH = '.codian/obsidian';
 ...
 * Merges CC settings (permissions) with Codian settings.
 */
export interface CombinedSettings {
  /** Runtime settings (permissions, etc.) */
  cc: CCSettings;
...
interface LegacySettingsJson {
  // Old Codian fields that were in settings.json
  userName?: string;
...
// CODIAN_ONLY_FIELDS is imported from ./migrationConstants
...
export class StorageService {
  ...
  private async runMigrations(): Promise<void> {
    ...
    const codianExists = await this.codianSettings.exists();
    const dataJson = await this.loadDataJson();

    // Check if old settings.json has Codian fields that need migration
    if (ccExists && !codianExists) {
  ...
  private hasStateToMigrate(data: LegacyDataJson): boolean {
    return (
      ...
      data.lastPresetModel !== undefined ||
  ...
  /**
   * Migrate from old settings.json (with Codian fields) to split format.
   *
   * Handles:
   * - Legacy Codian fields (userName, model, etc.) → codian-settings.json
   * - Legacy permissions array → CC permissions object
   * - CC env object → Codian environmentVariables string
  ...
  private async migrateFromOldSettingsJson(): Promise<void> {
    ...
    const hasCodianFields = Array.from(CODIAN_ONLY_FIELDS).some(
    ...
    // Handle environment variables: merge Codian string format with CC object format
    ...
    const codianFields: Partial<StoredCodianSettings> = {
      ...
      lastPresetModel: DEFAULT_SETTINGS.lastPresetModel,
      lastCustomModel: DEFAULT_SETTINGS.lastCustomModel,
      lastEnvHash: DEFAULT_SETTINGS.lastEnvHash,
    };

    // Save Codian settings FIRST (before stripping from settings.json)
    await this.codianSettings.save(codianFields as StoredCodianSettings);

    // Verify Codian settings were saved
    const savedCodian = await this.codianSettings.load();
    if (!savedCodian || savedCodian.userName === undefined) {
    ...
    const ccSettings: CCSettings = {
      $schema: 'https://openai.com/codex/runtime-settings.schema.json',
      permissions: ccPermissions,
    };

    // Pass true to strip Codian-only fields during migration
  ...
  private async migrateFromDataJson(dataJson: LegacyDataJson): Promise<void> {
    ...
    if (dataJson.lastPresetModel !== undefined && !codian.lastPresetModel) {
      codian.lastPresetModel = dataJson.lastPresetModel;
    }
  ...
  private async clearLegacyDataJson(): Promise<void> {
    ...
    delete cleaned.lastPresetModel;
