 ...
 * 2. Vault agents: {vaultPath}/.codex/agents/*.md
 * 3. Global agents: ~/.codex/agents/*.md
 */
...
const GLOBAL_AGENTS_DIRS = [path.join(os.homedir(), '.codex', 'agents')];
const VAULT_AGENTS_DIRS = ['.codex/agents'];
...
export class AgentManager {
  ...
  private async loadGlobalAgents(): Promise<void> {
    for (const globalAgentsDir of GLOBAL_AGENTS_DIRS) {
      await this.loadAgentsFromDirectory(globalAgentsDir, 'global');
    }
  }
