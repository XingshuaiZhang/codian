...
import { getContextWindowSize, VIEW_TYPE_CODIAN } from '../../core/types';
import type CodianPlugin from '../../main';
import { PLUGIN_BRAND_NAME, PLUGIN_ICON_ID, PLUGIN_LOGO_SVG } from '../../shared/brand';
...
export class CodianView extends ItemView {
  ...
  refreshModelSelector(): void {
    ...
    for (const tab of this.tabManager?.getAllTabs() ?? []) {
      if (tab.state.usage && !tab.state.usage.isContextWindowAuthoritative) {
  ...
  private buildHeader(header: HTMLElement) {
    ...
    this.titleTextEl = this.titleSlotEl.createEl('h4', { text: PLUGIN_BRAND_NAME, cls: 'codian-title-text' });
  ...
  private wireEventHandlers(): void {
    ...
    // navigating away when Codian is open as a main-area tab.
