...
import type { CodianService } from '../../../core/agent';
...
export class TabManager implements TabManagerInterface {
  ...
  private async createForkConversation(context: ForkContext): Promise<string> {
    ...
    await this.plugin.updateConversation(conversation.id, {
      ...
      ...(context.externalContextPaths?.length ? { externalContextPaths: context.externalContextPaths } : {}),
      ...(context.enabledMcpServers?.length ? { enabledMcpServers: context.enabledMcpServers } : {}),
    });
   ...
   */
  async broadcastToAllTabs(fn: (service: CodianService) => Promise<void>): Promise<void> {
