...
import { CodianService } from '../../../core/agent';
import type { McpServerManager } from '../../../core/mcp';
import type { ChatMessage, CodexModel, Conversation, EffortLevel, PermissionMode, SlashCommand, StreamChunk, ThinkingBudget } from '../../../core/types';
import { DEFAULT_CODEX_MODELS, DEFAULT_EFFORT_LEVEL, DEFAULT_THINKING_BUDGET, getContextWindowSize, isAdaptiveThinkingModel } from '../../../core/types';
import { t } from '../../../i18n';
...
export async function initializeTabService(
  ...
  let service: CodianService | null = null;
  let unsubscribeReadyState: (() => void) | null = null;

  try {
    // Create per-tab CodianService
  ...
  if (plugin.settings.enableBangBash) {
    ...
    if (vaultPath) {
      ...
      const bashService = new BangBashService(vaultPath, enhancedPath, {
        runtimeMode: process.platform === 'win32' ? 'wsl' : (plugin.settings.codexRuntimeMode ?? 'native'),
        wslDistribution: plugin.settings.wslDistribution,
...
function initializeInputToolbar(tab: TabData, plugin: CodianPlugin): void {
  ...
  const toolbarComponents = createInputToolbar(inputToolbar, {
    ...
    onModelChange: async (model: CodexModel) => {
      const previousModel = plugin.settings.model;
      ...
      if (isDefaultModel) {
        ...
        plugin.settings.lastPresetModel = model;
      } else {
      ...
      if (modelChanged) {
        ...
        tab.service?.setSessionId(null, externalContextPaths);
        tab.state.usage = null;

        if (tab.state.currentConversationId) {
          await plugin.updateConversation(tab.state.currentConversationId, {
            sessionId: null,
      ...
      if (currentUsage && !currentUsage.isContextWindowAuthoritative) {
