...
import type { ApprovalCallbackOptions, CodianService } from '../../../core/agent';
...
export interface InputControllerDeps {
  ...
  getAgentService?: () => CodianService | null;
...
export class InputController {
  ...
  private getAgentService(): CodianService | null {
    ...
    try {
      ...
      if (!wasInvalidated && state.streamGeneration === streamGeneration) {
        ...
        if (didCancelThisTurn && !state.pendingNewSessionPlan) {
          await streamController.appendText('\n\n<span class="codian-interrupted">Interrupted</span> <span class="codian-interrupted-hint">· What should Codex do instead?</span>');
        }
        streamController.hideThinkingIndicator();
        state.isStreaming = false;
        state.cancelRequested = false;
