...
export class McpSettingsManager {
  ...
  private async loadAndRender() {
    this.servers = await this.plugin.loadAllMcpServers();
    this.render();
  }

  private getPersistedServers(): CodianMcpServer[] {
    return this.servers.filter((server) => server.source === 'vault');
  }
  ...
  private renderServerItem(listEl: HTMLElement, server: CodianMcpServer) {
    ...
    if (server.source && server.source !== 'vault') {
      const sourceEl = nameRow.createSpan({ cls: 'codian-mcp-type-badge' });
      sourceEl.setText(server.source);
      sourceEl.setAttribute('title', server.source === 'project'
        ? 'Loaded from project .codex/config.toml'
        : 'Loaded from Codex global config');
    }
    ...
    if (server.source === 'vault' || !server.source) {
  ...
  private async updateServerDisabledTools(
    ...
    try {
      await this.plugin.storage.mcp.save(this.getPersistedServers());
    } catch (error) {
