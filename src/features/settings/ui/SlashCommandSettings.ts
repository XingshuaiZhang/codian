...
export class SlashCommandSettings {
  ...
  private renderCommandItem(listEl: HTMLElement, cmd: SlashCommand): void {
    ...
    if (cmd.source === 'global') {
      headerRow.createSpan({ text: 'global', cls: 'codian-slash-item-badge' });
    }
    ...
    if (!isSkill(cmd) && cmd.source !== 'global') {
  ...
  private async reloadCommands(): Promise<void> {
    this.plugin.settings.slashCommands = await this.plugin.loadAllSlashCommands();
  }
