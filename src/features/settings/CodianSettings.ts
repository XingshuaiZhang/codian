...
import { getModelCatalogSnapshot } from '../../utils/modelCatalog';
...
function openHotkeySettings(app: App): void {
  ...
  setTimeout(() => {
    ...
    if (tab) {
      ...
      if (searchEl) {
        searchEl.value = PLUGIN_BRAND_NAME;
...
export class CodianSettingTab extends PluginSettingTab {
  ...
  display(): void {
        ...
        .addDropdown((dropdown) => {
          const applyCatalog = () => {
            const catalog = getModelCatalogSnapshot(
              ...
              this.plugin.settings.titleGenerationModel || undefined
            );
    ...
    const agentsDesc = containerEl.createDiv({ cls: 'codian-sp-settings-desc' });
    agentsDesc.createEl('p', {
      text: t('settings.subagents.desc'),
      cls: 'setting-item-description',
    });

    const agentsContainer = containerEl.createDiv({ cls: 'codian-agents-container' });
    new AgentSettings(agentsContainer, this.plugin);

    new Setting(containerEl).setName(t('settings.mcpServers.name')).setHeading();

    const mcpDesc = containerEl.createDiv({ cls: 'codian-mcp-settings-desc' });
    mcpDesc.createEl('p', {
      text: t('settings.mcpServers.desc'),
      cls: 'setting-item-description',
    });

    const mcpContainer = containerEl.createDiv({ cls: 'codian-mcp-container' });
    ...
    new Setting(containerEl)
      ...
      .addTextArea((text) => {
        text
          .setPlaceholder('HTTP_PROXY=http://127.0.0.1:7890\nHTTPS_PROXY=http://127.0.0.1:7890\nCODEX_HOME=~/.codex')
        ...
        text.inputEl.addEventListener('blur', async () => {
          ...
          text.setValue(this.plugin.settings.environmentVariables);
    ...
    const platformDesc = isWindowsHost
      ? t('settings.acpPath.descWindows')
      : t('settings.acpPath.descUnix');
    const acpPathDescription = `${t('settings.acpPath.desc')} ${platformDesc}`;
    const cliPlatformDesc = isWindowsHost
      ? t('settings.cliPath.descWindows')
      : t('settings.cliPath.descUnix');
    const cliPathDescription = `${t('settings.cliPath.desc')} ${cliPlatformDesc}`;
    ...
    const restartAllTabs = async (): Promise<void> => {
    ...
    const addPathSetting = (
      name: string,
      description: string,
      placeholder: string,
      currentValue: string,
      onUpdate: (trimmed: string) => Promise<void>,
    ): void => {
    ...
    const currentAcpValue = isWindowsHost
      ? (this.plugin.settings.wslCodexAcpPath || 'codex-acp')
      : (
          this.plugin.settings.codexAcpPathsByHost?.[hostnameKey]
          || this.plugin.settings.codexAcpPath
          || ''
        );

    addPathSetting(
      `${t('settings.acpPath.name')} (${hostnameKey})`,
      acpPathDescription,
      isWindowsHost ? 'codex-acp' : '/usr/local/bin/codex-acp',
      currentAcpValue,
      async (trimmed) => {
        if (isWindowsHost) {
          this.plugin.settings.wslCodexAcpPath = trimmed || 'codex-acp';
        } else {
          this.plugin.settings.codexAcpPathsByHost ??= {};
          this.plugin.settings.codexAcpPathsByHost[hostnameKey] = trimmed;
        }
        await this.plugin.saveSettings();
        this.plugin.acpCliResolver?.reset();
        await restartAllTabs();
      },
    );

    const currentCliValue = isWindowsHost
      ? (this.plugin.settings.wslCodexPath || 'codex')
      : (
          this.plugin.settings.codexCliPathsByHost?.[hostnameKey]
          || this.plugin.settings.codexCliPath
          || ''
        );

    addPathSetting(
      `${t('settings.cliPath.name')} (${hostnameKey})`,
      cliPathDescription,
      isWindowsHost ? 'codex' : '/usr/local/bin/codex',
      currentCliValue,
      async (trimmed) => {
        if (isWindowsHost) {
          this.plugin.settings.wslCodexPath = trimmed || 'codex';
        } else {
    ...
    if (isWindowsHost) {
      new Setting(containerEl)
        ...
        .setDesc('Windows hosts are supported only through WSL2.')
        .addDropdown((dropdown) => {
          dropdown
            .addOption('wsl', 'WSL2')
            .setValue('wsl')
            .onChange(async () => {
              this.plugin.settings.codexRuntimeMode = 'wsl';
              await this.plugin.saveSettings();
            });
        });
      ...
      new Setting(containerEl)
        ...
        .setDesc('Optional. Leave empty to use the default WSL distribution.')
        .addText((text) => {
          text
