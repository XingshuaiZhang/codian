...
import type { HookCallbackMatcher } from '../../core/types/sdk';
  ...
  return {
    hooks: [
      async (hookInput) => {
        ...
        // Allows vault and approved Codex config paths (context/readwrite params are undefined)
        ...
        return {
          ...
          hookSpecificOutput: {
            hookEventName: 'PreToolUse' as const,
            permissionDecision: 'deny' as const,
            permissionDecisionReason: `Access denied: Path "${filePath}" is outside allowed paths. Inline edit is restricted to the vault and approved Codex configuration directories.`,
          },
        };
      },
    ],
    ...
    try {
      const response = await runCodexText({
        ...
        prompt: [
          ...
          `</system_instructions>`,
          '',
          `Available read-only tools: ${READ_ONLY_TOOLS.join(', ')}.`,
          `Do not modify files or run write commands. Only inspect context and return the requested text snippet.`,
          '',
          prompt,
        ].join('\n'),
      ...
      return parseInlineEditResponse(response.text);
    } catch (error) {
