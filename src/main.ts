/**
 * Codex CLI - Obsidian plugin entry point
 *
...
import { PLUGIN_BRAND_NAME, PLUGIN_ICON_CONTENT, PLUGIN_ICON_ID } from './shared/brand';
import { CodexAcpResolver } from './utils/codexAcpResolver';
import { CodexCliResolver } from './utils/codexCliResolver';
import {
...
/**
 * Main plugin class for the Codex CLI Obsidian integration.
...
export default class CodianPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.acpCliResolver = new CodexAcpResolver();
    ...
    this.addRibbonIcon(PLUGIN_ICON_ID, `Open ${PLUGIN_BRAND_NAME}`, () => {
      this.activateView();
    });
  ...
  async loadSettings() {
    ...
    let didNormalizeModelVariants = this.normalizeModelVariantSettings();

    let didMigrateCliPath = false;

    this.settings.codexCliPathsByHost ??= {};
    this.settings.codexAcpPathsByHost ??= {};
    const hostname = getHostnameKey();
    if (this.settings.codexCliPath?.trim() && !this.settings.codexCliPathsByHost[hostname]) {
      this.settings.codexCliPathsByHost[hostname] = this.settings.codexCliPath.trim();
    ...
    const {
      envText: sanitizedEnvText,
      removedKeys: removedAuthKeys,
    } = sanitizeUnsupportedCodexAuthEnvironmentText(this.settings.environmentVariables || '');
    if (sanitizedEnvText !== (this.settings.environmentVariables || '')) {
      this.settings.environmentVariables = sanitizedEnvText;
      didNormalizeModelVariants = true;
    }
    ...
    if (removedAuthKeys.length > 0) {
      new Notice('OPENAI_API_KEY and OPENAI_BASE_URL are no longer supported in Codian. Use `codex login` or `codex login --with-api-key` in your terminal.');
    }
  ...
  normalizeModelVariantSettings(): boolean {
    ...
    const allowedModels = new Set(DEFAULT_CODEX_MODELS.map((model) => model.value));
    let changed = false;

    const normalize = (model: string, fallback: string): string => {
      const normalized = normalizeVisibleModelVariant(model, enableOpus1M, enableSonnet1M);
      return allowedModels.has(normalized) ? normalized : fallback;
    };

    const normalizedModel = normalize(this.settings.model, DEFAULT_CODEX_MODELS[0].value);
    if (this.settings.model !== normalizedModel) {
      this.settings.model = normalizedModel;
      changed = true;
    }

    const normalizedTitleModel = this.settings.titleGenerationModel
      ? normalize(this.settings.titleGenerationModel, '')
      : '';
    ...
    if (this.settings.lastPresetModel) {
      const normalizedLastPresetModel = normalize(this.settings.lastPresetModel, DEFAULT_CODEX_MODELS[0].value);
      if (this.settings.lastPresetModel !== normalizedLastPresetModel) {
        this.settings.lastPresetModel = normalizedLastPresetModel;
        changed = true;
      }
    }

    if (this.settings.lastCustomModel) {
  ...
  async applyEnvironmentVariables(envText: string): Promise<void> {
    const { envText: sanitizedEnvText, removedKeys } = sanitizeUnsupportedCodexAuthEnvironmentText(envText);
    const envChanged = sanitizedEnvText !== this.runtimeEnvironmentVariables;
    ...
    const { changed, invalidatedConversations } = this.reconcileModelWithEnvironment(sanitizedEnvText);
    await this.saveSettings();
    ...
    if (removedKeys.length > 0) {
      notices.push('OPENAI_API_KEY and OPENAI_BASE_URL are no longer supported in Codian. Use `codex login` or `codex login --with-api-key` in your terminal.');
    }
    notices.push(
      changed
  ...
  async loadAllSlashCommands(): Promise<SlashCommand[]> {
    ...
    const externalSkills = await loadExternalCodexSkills({
      ...
      environmentVariables: this.getActiveEnvironmentVariables(),
    });
    return mergeSlashCommands(localCommands, externalSkills);
  }

  async loadAllMcpServers(): Promise<CodianMcpServer[]> {
    const vaultServers = (await this.storage.mcp.load()).map((server) => ({
      ...server,
    ...
    return mergeMcpServers(mergeMcpServers(vaultServers, projectServers), externalServers);
  }

  getResolvedCodexCliPath(): string | null {
    if (process.platform === 'win32') {
      return this.settings.wslCodexPath?.trim() || 'codex';
    }
  ...
  getResolvedCodexAcpPath(): string | null {
    if (process.platform === 'win32') {
      return this.settings.wslCodexAcpPath?.trim() || 'codex-acp';
    }

    return this.acpCliResolver.resolve(
      this.settings.codexAcpPathsByHost,
      this.settings.codexAcpPath,
      this.getActiveEnvironmentVariables()
    );
  }

  /** Computes a hash for environment changes that should invalidate persisted sessions. */
  private computeEnvHash(envText: string): string {
    const envVars = parseEnvironmentVariables(envText || '');
    const unsupportedKeys = findUnsupportedCodexAuthEnvironmentKeys(envVars);
    if (unsupportedKeys.length > 0) {
      return unsupportedKeys.sort().join('|');
    }
    return '';
  }
  ...
  private async loadSdkMessagesForConversation(conversation: Conversation): Promise<void> {
    if (conversation.sdkMessagesLoaded) return;

    if (conversation.subagentData) {
      this.applySubagentData(conversation.messages, conversation.subagentData);
    }
