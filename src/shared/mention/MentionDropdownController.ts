  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        ...
        switch (item.type) {
          case 'mcp-server':
            textEl.createSpan({ cls: 'codian-mention-name' }).setText(`@${item.name}`);
            break;
          case 'agent-folder':
            textEl.createSpan({
          ...
          case 'agent': {
            ...
            textEl.createSpan({
              cls: 'codian-mention-name codian-mention-name-agent',
            }).setText(`@${item.id}`);
            if (item.description) {
              textEl.createSpan({ cls: 'codian-mention-agent-desc' }).setText(item.description);
            }
            break;
          }
          case 'context-folder':
