/**
 * SDK Session Parser - Parses Codex CLI runtime native session files.
 *
 * The SDK stores sessions in ~/.codex/projects/{vault-path-encoded}/{sessionId}.jsonl
 * Each line is a JSON object with message data.
 *
 * This utility converts SDK native messages to Codian's ChatMessage format
...
/** Stored in session JSONL files. Based on Codex CLI runtime internal format. */
 ...
 *
 * File location:
 * ~/.codex/projects/{encoded-vault}/{sessionId}/subagents/agent-{agentId}.jsonl
 */
 ...
 * Common formats: SDK UUIDs, Codian IDs (conv-TIMESTAMP-RANDOM).
 */
