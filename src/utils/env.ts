...
export function resolveWindowsWslExecutable(): string {
  ...
  const addWindowsRoot = (root: string | undefined): void => {
    if (!root) return;
    const normalizedRoot = root.trim().replace(/[\\/]+$/, '');
    if (!normalizedRoot) return;
    addCandidate(path.win32.join(normalizedRoot, 'System32', 'wsl.exe'));
    addCandidate(path.win32.join(normalizedRoot, 'Sysnative', 'wsl.exe'));
  };

  addWindowsRoot(process.env.SystemRoot);
  addWindowsRoot(process.env.SYSTEMROOT);
  addWindowsRoot(process.env.windir);
  addWindowsRoot(process.env.WINDIR);

  const comspec = process.env.ComSpec || process.env.COMSPEC;
  if (comspec) {
    const comspecDir = path.win32.dirname(comspec.trim());
    addCandidate(path.win32.join(comspecDir, 'wsl.exe'));
    addWindowsRoot(path.win32.dirname(comspecDir));
  }
  ...
  for (const entry of rawPath.split(';')) {
    ...
    addCandidate(path.win32.join(trimmedEntry, 'wsl.exe'));
  }

  for (let code = 67; code <= 90; code++) {
    const drive = `${String.fromCharCode(code)}:\\Windows`;
    addWindowsRoot(drive);
  }
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Ignore inaccessible candidates and fall back to PATH lookup.
    }
  }
...
export function withProxyAliases<T extends Record<string, string | undefined>>(input: T): T {
  const result = { ...input } as T;
  const writable = result as Record<string, string | undefined>;

  for (const [upperKey, lowerKey] of PROXY_ENV_PAIRS) {
    const upperValue = writable[upperKey];
    const lowerValue = writable[lowerKey];

    if (upperValue && !lowerValue) {
      writable[lowerKey] = upperValue;
    } else if (lowerValue && !upperValue) {
      writable[upperKey] = lowerValue;
    }
  }
...
export function getWslForwardedEnvironment(envVars: Record<string, string>): Record<string, string> {
  ...
  return withProxyAliases(forwarded);
}
