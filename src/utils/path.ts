...
export function findCodexCLIPath(pathValue?: string): string | null {
  ...
  // On Windows, prefer native .exe, then cli.js.
  if (isWindows) {
...
export function findNamedUnixCLIPath(binaryName: string, pathValue?: string): string | null {
  const homeDir = os.homedir() || process.env.HOME || process.env.USERPROFILE || '';
  const customEntries = dedupePaths(parsePathEntries(pathValue));

  if (customEntries.length > 0) {
    const customResolution = findFirstExistingPath(customEntries, [binaryName]);
    if (customResolution) {
      return customResolution;
    }
  }
  ...
  const commonPaths: string[] = [
    ...
    path.join(homeDir, '.asdf', 'bin', binaryName),
  ...
  const npmPrefix = getNpmGlobalPrefix();
  if (npmPrefix) {
    commonPaths.push(path.join(npmPrefix, 'bin', binaryName));
  }

  const nvmBin = resolveNvmDefaultBin(homeDir);
  if (nvmBin) {
    commonPaths.push(path.join(nvmBin, binaryName));
  }
  ...
  const envEntries = dedupePaths(parsePathEntries(getEnvValue('PATH')));
  if (envEntries.length > 0) {
    const envResolution = findFirstExistingPath(envEntries, [binaryName]);
    if (envResolution) {
      return envResolution;
    }
  }
...
export function translateWindowsPathToWsl(inputPath: string): string | null {
  if (!inputPath || typeof inputPath !== 'string') {
    return null;
  }

  const uncMatch = inputPath.match(/^\\\\wsl\$\\([^\\]+)\\(.+)$/i);
  if (uncMatch) {
    const [, distro, rest] = uncMatch;
    const wslPath = `/${rest.replace(/\\/g, '/')}`;
    return `${distro}:${wslPath}`;
  }
...
interface SafeHomePathRule {
  root: string;
  safeSubdirs: string[];
  safeFiles: string[];
}

function getSafeHomePathRules(): SafeHomePathRule[] {
  return [
    {
      root: path.join(os.homedir(), '.codian'),
      safeSubdirs: ['obsidian'],
      safeFiles: [],
    },
    {
      root: path.join(os.homedir(), '.codex'),
      safeSubdirs: ['agents', 'commands', 'obsidian', 'plans', 'prompts', 'sessions', 'skills'],
      safeFiles: ['auth.json', 'config.toml', 'settings.json', 'settings.local.json'],
    },
    {
      root: path.join(os.homedir(), '.agents'),
      safeSubdirs: ['skills'],
      safeFiles: [],
    },
  ];
...
function getSafeHomePathAccess(resolvedCandidate: string): PathAccessType | null {
  for (const rule of getSafeHomePathRules()) {
    const resolvedRoot = normalizePathForComparison(resolveRealPath(rule.root));
    if (
    ...
    const relative = resolvedCandidate.slice(resolvedRoot.length + 1);
    if (!relative) {
      return 'context';
    }

    const topSegment = relative.split('/')[0];
    if (rule.safeSubdirs.includes(topSegment) || rule.safeFiles.includes(topSegment)) {
      return 'vault';
    }
...
export function getPathAccessType(
  ...
  const safeHomePathAccess = getSafeHomePathAccess(resolvedCandidate);
  if (safeHomePathAccess) {
    return safeHomePathAccess;
  }
