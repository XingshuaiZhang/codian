...
import type { SpawnOptions } from '@/core/types/sdk';
...
describe('createCustomSpawnFunction', () => {
  ...
  it('pipes stderr only when DEBUG_CODEX_RUNTIME is set', () => {
    const mockProcess = createMockProcess();
    spawnMock.mockReturnValue(mockProcess as unknown as ReturnType<typeof spawn>);
  ...
  it('ignores stderr when DEBUG_CODEX_RUNTIME is not set', () => {
    const mockProcess = createMockProcess();
    spawnMock.mockReturnValue(mockProcess as unknown as ReturnType<typeof spawn>);
