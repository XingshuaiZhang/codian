...
describe('SkillStorage', () => {
  it('exports SKILLS_PATH', () => {
    expect(SKILLS_PATH).toBe('.agents/skills');
  });
...
Prompt B`,
    ...
    it('skips folders without SKILL.md', async () => {
      const adapter = createMockAdapter({
        '.agents/skills/has-skill/SKILL.md': `---
description: Valid
---
Prompt`,
        '.agents/skills/no-skill/README.md': 'Just a readme',
      });
      const storage = new SkillStorage(adapter);
      const skills = await storage.loadAll();
...
---
Prompt`,
        '.agents/skills/bad/SKILL.md': 'content',
      });
...
---
Prompt`,
        '.agents/skills/invalid/SKILL.md': 'No frontmatter at all',
      });
      const storage = new SkillStorage(adapter);
      const skills = await storage.loadAll();
  ...
  describe('save', () => {
    it('writes skill to correct path', async () => {
      ...
      expect(adapter.ensureFolder).toHaveBeenCalledWith('.agents/skills/my-skill');
