...
describe('SessionStorage', () => {
  ...
  describe('SESSIONS_PATH', () => {
    it('should be .codian/obsidian/sessions', () => {
      expect(SESSIONS_PATH).toBe('.codian/obsidian/sessions');
    });
  });
  ...
  describe('getFilePath', () => {
    it('returns correct file path for conversation id', () => {
      ...
      expect(path).toBe('.codian/obsidian/sessions/conv-123.jsonl');
    });
  });
  ...
  describe('loadConversation', () => {
    it('returns null if file does not exist', async () => {
      ...
      expect(mockAdapter.exists).toHaveBeenCalledWith('.codian/obsidian/sessions/conv-123.jsonl');
    });
  ...
  describe('saveConversation', () => {
    it('serializes conversation to JSONL and writes to file', async () => {
      ...
      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.codian/obsidian/sessions/conv-456.jsonl',
        expect.any(String)
      );
  ...
  describe('deleteConversation', () => {
    it('deletes the JSONL file', async () => {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith('.codian/obsidian/sessions/conv-del.jsonl');
    });
  });
  ...
  describe('listConversations', () => {
    it('returns metadata for all JSONL files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/conv-1.jsonl',
        '.codex/sessions/conv-2.jsonl',
        '.codex/sessions/readme.txt', // Should be skipped
      ]);
    ...
    it('skips files that fail to load', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/good.jsonl',
        '.codex/sessions/bad.jsonl',
      ]);
    ...
    it('truncates long previews', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codex/sessions/conv-long.jsonl']);
    ...
    it('uses default preview for conversations without user messages', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codex/sessions/conv-no-user.jsonl']);
    ...
    it('preserves titleGenerationStatus in meta', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codex/sessions/conv-status.jsonl']);
  ...
  describe('loadAllConversations', () => {
    it('loads full conversation data for all JSONL files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/conv-a.jsonl',
        '.codex/sessions/conv-b.jsonl',
      ]);
    ...
    it('skips non-JSONL files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/conv.jsonl',
        '.codex/sessions/notes.md',
  ...
  describe('getMetadataPath', () => {
    it('returns correct file path for session id', () => {
      ...
      expect(path).toBe('.codian/obsidian/sessions/session-abc.meta.json');
    });
  });
  ...
  describe('saveMetadata', () => {
    it('serializes metadata to JSON and writes to file', async () => {
      ...
      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.codian/obsidian/sessions/session-456.meta.json',
        expect.any(String)
      );
  ...
  describe('deleteMetadata', () => {
    it('deletes the meta.json file', async () => {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith('.codian/obsidian/sessions/session-del.meta.json');
    });
  });
  ...
  describe('listNativeMetadata', () => {
    it('returns metadata for .meta.json files without .jsonl counterparts', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/native-1.meta.json',
        '.codex/sessions/native-2.meta.json',
        '.codex/sessions/legacy.jsonl',
        '.codex/sessions/legacy.meta.json', // Has JSONL counterpart, skip
      ]);

      // native-1.meta.json and native-2.meta.json have no .jsonl
      // legacy.meta.json has .jsonl counterpart
      mockAdapter.exists.mockImplementation((path: string) => {
        if (path === '.codex/sessions/native-1.jsonl') return Promise.resolve(false);
        if (path === '.codex/sessions/native-2.jsonl') return Promise.resolve(false);
        if (path === '.codex/sessions/legacy.jsonl') return Promise.resolve(true);
    ...
    it('skips files that fail to load', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/good.meta.json',
        '.codex/sessions/bad.meta.json',
      ]);
  ...
  describe('listAllConversations', () => {
    it('merges legacy and native conversations', async () => {
      ...
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/legacy-1.jsonl',
        '.codex/sessions/native-1.meta.json',
      ]);

      mockAdapter.exists.mockImplementation((path: string) => {
        // native-1 has no .jsonl counterpart
        if (path === '.codian/obsidian/sessions/native-1.jsonl') return Promise.resolve(false);
        if (path === '.codex/sessions/native-1.jsonl') return Promise.resolve(false);
    ...
    it('legacy takes precedence over native with same ID', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/conv-1.jsonl',
        '.codex/sessions/conv-1.meta.json', // Same ID, should be skipped
      ]);
    ...
    it('native sessions have isNative flag and default preview', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/sessions/native-only.meta.json',
      ]);
  ...
  describe('listConversations - loadMetaOnly edge cases', () => {
    it('handles corrupted message lines in loadMetaOnly', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codex/sessions/corrupted.jsonl']);
    ...
    it('returns empty when first line (meta) parse fails', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codex/sessions/bad-meta.jsonl']);
