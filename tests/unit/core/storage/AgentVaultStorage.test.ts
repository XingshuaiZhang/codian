...
Run the tests.`;
  ...
  describe('loadAll', () => {
    ...
    it('continues loading if one file throws', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/agents/good.md',
        '.codex/agents/error.md',
        '.codex/agents/also-good.md',
      ]);
      mockAdapter.read
        .mockResolvedValueOnce(validAgentMd)
    ...
    it('preserves filePath from disk', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codex/agents/custom-filename.md',
      ]);
      mockAdapter.read.mockResolvedValue(validAgentMd);

      const agents = await storage.loadAll();

      expect(agents[0].filePath).toBe('.codex/agents/custom-filename.md');
    });
...
Be strict.`;
      ...
      mockAdapter.listFiles.mockResolvedValue(['.codex/agents/strict-agent.md']);
  ...
  describe('delete', () => {
    it('deletes using filePath when available', async () => {
      await storage.delete({
        ...
        prompt: 'Review.',
        source: 'vault',
        filePath: '.codex/agents/custom-filename.md',
      });

      expect(mockAdapter.delete).toHaveBeenCalledWith('.codex/agents/custom-filename.md');
    });
    ...
    it('falls back to name-based path when no filePath', async () => {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith('.codex/agents/code-reviewer.md');
    });
    ...
    it('converts absolute filePath to vault-relative', async () => {
      await storage.delete({
        ...
        prompt: 'Review.',
        source: 'vault',
        filePath: '/Users/user/vault/.codex/agents/code-reviewer.md',
      });
    ...
    it('converts Windows absolute filePath to vault-relative', async () => {
      await storage.delete({
        ...
        prompt: 'Review.',
        source: 'vault',
        filePath: 'C:\\Users\\user\\vault\\.codex\\agents\\custom-filename.md',
      });
  ...
  describe('load', () => {
    it('reads and parses a single agent file', async () => {
      ...
      expect(mockAdapter.read).toHaveBeenCalledWith('.codex/agents/code-reviewer.md');
    ...
    it('resolves Windows absolute filePath before reading', async () => {
      ...
      expect(mockAdapter.read).toHaveBeenCalledWith('.codex/agents/custom-filename.md');
    });
  });
  ...
  describe('path normalization', () => {
    it('saves using vault-relative path when filePath is absolute', async () => {
      await storage.save({
        ...
        prompt: 'Do stuff.',
        source: 'vault',
        filePath: '/Users/user/vault/.codex/agents/my-agent.md',
      });
    ...
    it('preserves vault-relative filePath as-is', async () => {
      await storage.save({
        ...
        prompt: 'Do stuff.',
        source: 'vault',
        filePath: '.codex/agents/custom-name.md',
      });

      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.codex/agents/custom-name.md',
        expect.any(String)
      );
    });
    ...
    it('saves using custom filename from Windows absolute path', async () => {
      await storage.save({
        ...
        prompt: 'Do stuff.',
        source: 'vault',
        filePath: 'C:\\Users\\user\\vault\\.codex\\agents\\custom-name.md',
      });
