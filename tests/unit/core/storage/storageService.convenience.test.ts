...
describe('StorageService convenience methods', () => {
  ...
  describe('getPermissions', () => {
    it('delegates to ccSettings.getPermissions', async () => {
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.codian/obsidian/runtime-settings.json': ccSettingsJson,
        },
      });
  ...
  describe('updatePermissions', () => {
    it('saves updated permissions via ccSettings', async () => {
      ...
      const saved = JSON.parse(files.get('.codian/obsidian/runtime-settings.json')!) as Record<string, unknown>;
  ...
  describe('removePermissionRule', () => {
    it('removes a rule from all permission lists', async () => {
      ...
      const { plugin, files } = createMockPlugin({
        initialFiles: { '.codian/obsidian/runtime-settings.json': settings },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
  ...
  describe('updateCodianSettings', () => {
    it('updates partial codian settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.codian/obsidian/settings.json': codianSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
  ...
  describe('loadAllSlashCommands', () => {
    it('returns commands from both commands and skills directories', async () => {
      ...
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.codian/obsidian/commands/review.md': commandContent,
  ...
  describe('getLegacyActiveConversationId', () => {
    it('returns id from codian settings', async () => {
      ...
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.codian/obsidian/settings.json': settings,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
    ...
    it('falls back to data.json when not in codian settings', async () => {
      const { plugin } = createMockPlugin({
        ...
        initialFiles: {
          '.codian/obsidian/settings.json': JSON.stringify({ userName: 'Test' }),
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();
