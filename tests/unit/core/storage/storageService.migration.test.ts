...
describe('StorageService migration', () => {
  it('clears data.json after successful legacy content migration', async () => {
    ...
    expect(files.has('.codian/obsidian/commands/review.md')).toBe(true);
  ...
  it('does not clear data.json when legacy content migration fails', async () => {
    ...
    const { plugin } = createMockPlugin({
      dataJson: { slashCommands: [command] },
      shouldFailWrite: (path) => path.startsWith('.codian/obsidian/commands/'),
    });
  ...
  it('normalizes legacy blockedCommands during settings migration', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: null,
      initialFiles: {
        '.codian/obsidian/runtime-settings.json': JSON.stringify(legacySettings),
      },
    });
    ...
    const saved = JSON.parse(files.get('.codian/obsidian/settings.json') || '{}') as Record<string, unknown>;
  ...
  it('does not migrate legacy activeConversationId from data.json', async () => {
    ...
    const rawSettings = files.get('.codian/obsidian/settings.json');
  ...
  it('preserves CC-format permissions during migration', async () => {
    ...
    const ccSettings = JSON.parse(files.get('.codian/obsidian/runtime-settings.json') || '{}') as Record<string, any>;
  ...
  it('migrates data.json state fields to codian-settings when empty', async () => {
    ...
    // lastPresetModel already has a default value, so migration doesn't overwrite it
    expect(saved.lastPresetModel).toBe(DEFAULT_SETTINGS.lastPresetModel);
  ...
  it('does not overwrite existing codian-settings fields from data.json', async () => {
    const { plugin, files } = createMockPlugin({
      dataJson: {
        ...
        lastPresetModel: 'old-model',
      },
      initialFiles: {
        '.codian/obsidian/settings.json': JSON.stringify({
          userName: 'Test User',
          lastEnvHash: 'existing-hash',
          lastPresetModel: 'existing-model',
        }),
      },
    });
    ...
    expect(saved.lastPresetModel).toBe('existing-model');
  });
  ...
  it('skips existing slash commands during migration', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: { slashCommands: [command] },
      initialFiles: {
        '.codian/obsidian/commands/review.md': 'Existing content',
      },
    });
    ...
    expect(files.get('.codian/obsidian/commands/review.md')).toBe('Existing content');
  });
  ...
  it('migrates conversations from data.json', async () => {
    ...
    expect(files.has('.codian/obsidian/sessions/conv-1.jsonl')).toBe(true);
  });
  ...
  it('skips existing conversations during migration', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: { conversations: [conversation] },
      initialFiles: {
        '.codian/obsidian/sessions/conv-1.jsonl': '{"existing": true}',
      },
    });
    ...
    expect(files.get('.codian/obsidian/sessions/conv-1.jsonl')).toBe('{"existing": true}');
  });
  ...
  it('migrates lastPresetModel from data.json when codian-settings has falsy value', async () => {
    const { plugin, files } = createMockPlugin({
      dataJson: {
        lastPresetModel: 'gpt-5-codex',
      },
      initialFiles: {
        '.codian/obsidian/runtime-settings.json': JSON.stringify({
    ...
    const saved = JSON.parse(files.get('.codian/obsidian/settings.json') || '{}') as Record<string, unknown>;
    expect(saved.lastPresetModel).toBe('gpt-5-codex');
  });
  ...
  it('preserves persistentExternalContextPaths from existing settings', async () => {
    ...
    const { plugin, files } = createMockPlugin({
      dataJson: null,
      initialFiles: {
        '.codian/obsidian/settings.json': JSON.stringify(existingSettings),
      },
    });
