...
describe('types.ts', () => {
  ...
  describe('DEFAULT_SETTINGS', () => {
    ...
    it('should have lastPresetModel set to gpt-5.4 by default', () => {
      expect(DEFAULT_SETTINGS.lastPresetModel).toBe('gpt-5.4');
    });
  ...
  describe('CodianSettings type', () => {
    ...
    it('should accept optional lastPresetModel and lastCustomModel', () => {
      const settings: CodianSettings = {
        userName: '',
        enableBlocklist: true,
        allowExternalAccess: false,
      ...
      expect(settings.lastPresetModel).toBe('opus');
  ...
  describe('getContextWindowSize', () => {
    ...
    describe('[1m] suffix detection', () => {
      it('should not special-case legacy [1m] suffixes anymore', () => {
        expect(getContextWindowSize('gpt-5-codex[1m]')).toBe(CONTEXT_WINDOW_STANDARD);
        expect(getContextWindowSize('gpt-5[1m]')).toBe(CONTEXT_WINDOW_STANDARD);
      });

      it('should still prefer custom limits for suffix variants', () => {
        const customLimits = { 'gpt-5-codex[1m]': 500000 };
        expect(getContextWindowSize('gpt-5-codex[1m]', customLimits)).toBe(500000);
      });

      it('should return the official GPT-5 family windows for known models', () => {
        expect(getContextWindowSize('gpt-5.4')).toBe(CONTEXT_WINDOW_GPT54);
        expect(getContextWindowSize('gpt-5.4-pro')).toBe(CONTEXT_WINDOW_GPT54);
        expect(getContextWindowSize('gpt-5.3-codex')).toBe(CONTEXT_WINDOW_GPT5);
        expect(getContextWindowSize('gpt-5.2-codex')).toBe(CONTEXT_WINDOW_GPT5);
        expect(getContextWindowSize('gpt-5.2')).toBe(CONTEXT_WINDOW_GPT5);
        expect(getContextWindowSize('gpt-5')).toBe(CONTEXT_WINDOW_GPT5);
      });
    });

    describe('filterVisibleModelOptions', () => {
      const defaultModelValues = DEFAULT_CODEX_MODELS.map((model) => model.value);

      it('should expose the default Codex-oriented model list when toggles are disabled', () => {
        const models = filterVisibleModelOptions(DEFAULT_CODEX_MODELS, false, false).map((model) => model.value);
        expect(models).toEqual(defaultModelValues);
      });

      it('should ignore legacy 1M toggles when both are enabled', () => {
        const models = filterVisibleModelOptions(DEFAULT_CODEX_MODELS, true, true).map((model) => model.value);
        expect(models).toEqual(defaultModelValues);
      });

      it('should ignore legacy Opus toggle', () => {
        const models = filterVisibleModelOptions(DEFAULT_CODEX_MODELS, true, false).map((model) => model.value);
        expect(models).toEqual(defaultModelValues);
      });

      it('should ignore legacy Sonnet toggle', () => {
    ...
    describe('normalizeVisibleModelVariant', () => {
      it('should leave built-in variants unchanged', () => {
        expect(normalizeVisibleModelVariant('gpt-5-codex', true, true)).toBe('gpt-5-codex');
        expect(normalizeVisibleModelVariant('gpt-5', false, false)).toBe('gpt-5');
        expect(normalizeVisibleModelVariant('o4-mini', true, false)).toBe('o4-mini');
      });

      it('should leave unrelated model ids unchanged', () => {
        expect(normalizeVisibleModelVariant('', true, true)).toBe('');
        expect(normalizeVisibleModelVariant('gpt-5-codex[1m]', true, true)).toBe('gpt-5-codex[1m]');
  ...
  describe('isAdaptiveThinkingModel', () => {
    it('should return true for the built-in GPT-5 and Codex model catalog', () => {
      expect(isAdaptiveThinkingModel('gpt-5.4')).toBe(true);
      expect(isAdaptiveThinkingModel('gpt-5.3-codex')).toBe(true);
      expect(isAdaptiveThinkingModel('gpt-5-codex')).toBe(true);
      expect(isAdaptiveThinkingModel('gpt-5')).toBe(true);
      expect(isAdaptiveThinkingModel('o4-mini')).toBe(true);
    });

    it('should return true for matching GPT-5 and o-series model IDs', () => {
      expect(isAdaptiveThinkingModel('gpt-5-2026-01-15')).toBe(true);
      expect(isAdaptiveThinkingModel('gpt-5-codex-preview')).toBe(true);
      expect(isAdaptiveThinkingModel('o3')).toBe(true);
    });
    ...
    it('should return false for legacy Codex model IDs', () => {
      expect(isAdaptiveThinkingModel('legacy-sonnet-4-6-20250514')).toBe(false);
      expect(isAdaptiveThinkingModel('openai/generic-model')).toBe(false);
      expect(isAdaptiveThinkingModel('legacy-o4-mini-20251001')).toBe(false);
    });

    it('should return false for unrelated partial model IDs', () => {
      expect(isAdaptiveThinkingModel('legacy-haiku')).toBe(false);
      expect(isAdaptiveThinkingModel('legacy-sonnet')).toBe(false);
      expect(isAdaptiveThinkingModel('codex')).toBe(false);
    });
  });
});
