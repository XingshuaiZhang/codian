...
jest.mock('@/core/agent', () => ({
  CodianService: jest.fn().mockImplementation(() => ({
...
const createMockCodianService = (overrides?: {
...
describe('Tab - Service Initialization', () => {
  describe('initializeTabService', () => {
    ...
    it('should create CodianService on first initialization', async () => {
      const options = createMockOptions();
      const tab = createTab(options);
    ...
    it('should ensureReady without session ID (just spin up process)', async () => {
      ...
      const agentModule = jest.requireMock('@/core/agent') as { CodianService: jest.Mock };
      agentModule.CodianService.mockImplementationOnce(() => createMockCodianService({ ensureReady: mockEnsureReady }));
    ...
    it('should sync model selector ready state with service readiness', async () => {
      ...
      const agentModule = jest.requireMock('@/core/agent') as { CodianService: jest.Mock };
      agentModule.CodianService.mockImplementationOnce(() => createMockCodianService({ onReadyStateChange: mockOnReadyStateChange }));
...
describe('Tab - UI Callback Wiring', () => {
  ...
  describe('initializeTabUI callbacks', () => {
    ...
    it('invalidates the current conversation session when switching models', async () => {
      const plugin = createMockPlugin({
        updateConversation: jest.fn().mockResolvedValue(undefined),
      });
      const options = createMockOptions({ plugin });
      ...
      tab.service = createMockCodianService() as any;

      initializeTabUI(tab, plugin);

      const { createInputToolbar } = jest.requireMock('@/features/chat/ui') as {
        createInputToolbar: jest.Mock;
      };
      const toolbarCallbacks = createInputToolbar.mock.calls.at(-1)?.[1];

      mockExternalContextSelector.getExternalContexts.mockReturnValue(['/ctx/project']);

      await toolbarCallbacks.onModelChange('gpt-5');

      expect(plugin.settings.model).toBe('gpt-5');
      expect(plugin.saveSettings).toHaveBeenCalled();
      expect((tab.service as any).resetSession).toHaveBeenCalled();
      expect((tab.service as any).setSessionId).toHaveBeenCalledWith(null, ['/ctx/project']);
