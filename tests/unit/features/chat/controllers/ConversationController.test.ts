...
describe('ConversationController - Session Persistence', () => {
  let controller: ConversationController;
  let deps: ConversationControllerDeps;
  ...
  describe('save - session change detection', () => {
    it('should persist the latest sessionId when the runtime creates a new session', async () => {
      deps.state.currentConversationId = 'conv-1';
      deps.state.messages = [{ id: '1', role: 'user', content: 'test', timestamp: Date.now() }];
    ...
    it('should keep the existing sessionId when the runtime keeps using the same session', async () => {
      deps.state.currentConversationId = 'conv-1';
      deps.state.messages = [{ id: '1', role: 'user', content: 'test', timestamp: Date.now() }];
    ...
    it('should clear the saved sessionId after invalidation', async () => {
      deps.state.currentConversationId = 'conv-1';
      deps.state.messages = [{ id: '1', role: 'user', content: 'test', timestamp: Date.now() }];
      ...
      mockAgentService.consumeSessionInvalidation.mockReturnValue(true);
    ...
    it('should fall back to the conversation sessionId when the runtime has not captured one yet', async () => {
      deps.state.currentConversationId = 'conv-1';
      deps.state.messages = [{ id: '1', role: 'user', content: 'test', timestamp: Date.now() }];
      ...
      mockAgentService.getSessionId.mockReturnValue(null);
...
describe('ConversationController - Fork Session ID Isolation', () => {
  ...
  it('should not persist the fork source session as the fork conversation sessionId', async () => {
  ...
  it('should persist the first owned session ID captured for a fork', async () => {
    deps.state.currentConversationId = 'fork-conv';
    deps.state.messages = [{ id: '1', role: 'user', content: 'test', timestamp: Date.now() }];
  ...
  it('should allow normal session persistence when fork metadata is already cleared', async () => {
    deps.state.currentConversationId = 'fork-conv';
    deps.state.messages = [{ id: '1', role: 'user', content: 'test', timestamp: Date.now() }];
...
describe('ConversationController - switchTo fork path', () => {
  ...
  beforeEach(() => {
    ...
    mockAgentService = {
      getSessionId: jest.fn().mockReturnValue(null),
      setSessionId: jest.fn(),
      applyForkState: jest.fn((conv: any) => (!conv.sessionId && conv.forkSource ? null : conv.sessionId ?? conv.sdkSessionId ?? null)),
      consumeSessionInvalidation: jest.fn().mockReturnValue(false),
    };
    deps = createMockDeps({
      getAgentService: () => mockAgentService,
  ...
  it('should clear the active session when switching to a pending fork', async () => {
    deps.state.currentConversationId = 'old-conv';
    ...
    expect(mockAgentService.setSessionId).toHaveBeenCalledWith(null, expect.any(Array));
  });
