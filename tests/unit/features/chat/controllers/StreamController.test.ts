...
describe('StreamController - Plan Mode', () => {
  ...
  describe('capturePlanFilePath', () => {
    it('should capture plan file path from Write tool_use', async () => {
      ...
      await controller.handleStreamChunk(
        { type: 'tool_use', id: 'write-1', name: 'Write', input: { file_path: '/home/user/.codex/plans/plan.md' } },
        msg
      );

      expect(deps.state.planFilePath).toBe('/home/user/.codex/plans/plan.md');
    });
    ...
    it('should capture plan file path with Windows backslashes', async () => {
      ...
      await controller.handleStreamChunk(
        { type: 'tool_use', id: 'write-1', name: 'Write', input: { file_path: 'C:\\.codex\\plans\\plan.md' } },
        msg
      );

      expect(deps.state.planFilePath).toBe('C:\\.codex\\plans\\plan.md');
    });
    ...
    it('should not capture plan path from non-Write tools', async () => {
      ...
      await controller.handleStreamChunk(
        { type: 'tool_use', id: 'read-1', name: 'Read', input: { file_path: '/home/user/.codex/plans/plan.md' } },
        msg
      );
