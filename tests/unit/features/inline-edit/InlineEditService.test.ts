...
describe('InlineEditService', () => {
  ...
  describe('vault restriction hook', () => {
    ...
    it('should allow Read inside ~/.codex/ directory', async () => {
      // Mock os.homedir to return a known path
      jest.spyOn(os, 'homedir').mockReturnValue('/home/test');

      const hook = createVaultRestrictionHook('/test/vault/path');
      const res = await callHook(hook.hooks[0],
        { tool_name: 'Read', tool_input: { file_path: '/home/test/.codex/settings.json' } },
        'tool-4', {},
      );
    ...
    it('should allow Glob inside ~/.codex/ directory', async () => {
      jest.spyOn(os, 'homedir').mockReturnValue('/home/test');

      const hook = createVaultRestrictionHook('/test/vault/path');
      const res = await callHook(hook.hooks[0],
        { tool_name: 'Glob', tool_input: { pattern: '/home/test/.codex/**/*.md' } },
        'tool-5', {},
      );
    ...
    it('should still block paths outside vault and ~/.codex/', async () => {
      jest.spyOn(os, 'homedir').mockReturnValue('/home/test');
    ...
    it('should block path traversal via ~/.codex/../ to escape allowed directory', async () => {
      jest.spyOn(os, 'homedir').mockReturnValue('/home/test');

      const hook = createVaultRestrictionHook('/test/vault/path');
      const res = await callHook(hook.hooks[0],
        { tool_name: 'Read', tool_input: { file_path: '/home/test/.codex/../.ssh/id_rsa' } },
        'tool-7', {},
      );
  ...
  describe('editText', () => {
    ...
    it('should send read-only Codex prompt and parse replacement output', async () => {
      mockRunCodexText.mockResolvedValueOnce(
        createRunResult('<replacement>fixed</replacement>', 'inline-session-123')
      );
      ...
      expect(prompt).toContain('Do not modify files or run write commands');
      expect(prompt).toContain('<editor_selection path="test.md">');
    });

    it('should include the external access system prompt variant when enabled', async () => {
      mockPlugin.settings.allowExternalAccess = true;
      service = new InlineEditService(mockPlugin);
      mockRunCodexText.mockResolvedValueOnce(createRunResult('<replacement>fixed</replacement>'));
      ...
      expect(prompt).toContain('outside the vault');
    });

    it('should capture session ID for follow-up conversation', async () => {
      mockRunCodexText
        .mockResolvedValueOnce(createRunResult('What do you want to change?', 'inline-session-123'))
        .mockResolvedValueOnce(createRunResult('<replacement>fixed</replacement>', 'inline-session-123'));
      ...
      expect(mockRunCodexText.mock.calls[1][0]).toEqual(expect.objectContaining({
        sessionId: 'inline-session-123',
      }));
    });

    it('should return clarification response', async () => {
      mockRunCodexText.mockResolvedValueOnce(
        createRunResult('Could you clarify what "fix" means?')
      );
  ...
  describe('continueConversation', () => {
    ...
    it('should resume session on follow-up', async () => {
      mockRunCodexText
        .mockResolvedValueOnce(createRunResult('What do you want?', 'continue-session'))
        .mockResolvedValueOnce(createRunResult('<replacement>final result</replacement>', 'continue-session'));
    ...
    it('should prepend context files when provided', async () => {
      mockRunCodexText
        .mockResolvedValueOnce(createRunResult('What do you want?', 'context-session'))
        .mockResolvedValueOnce(createRunResult('<replacement>final result</replacement>', 'context-session'));
      ...
      const prompt = mockRunCodexText.mock.calls[1][0].prompt;
      expect(mockRunCodexText.mock.calls[1][0]).toEqual(expect.objectContaining({
    ...
    it('should not modify prompt when no context files provided', async () => {
      mockRunCodexText
        .mockResolvedValueOnce(createRunResult('What do you want?', 'no-context-session'))
        .mockResolvedValueOnce(createRunResult('<replacement>result</replacement>', 'no-context-session'));
    ...
    it('should handle empty context files array', async () => {
      mockRunCodexText
        .mockResolvedValueOnce(createRunResult('What do you want?', 'empty-context-session'))
        .mockResolvedValueOnce(createRunResult('<replacement>result</replacement>', 'empty-context-session'));
  ...
  describe('resetConversation', () => {
    it('should clear session so continueConversation fails', async () => {
      mockRunCodexText.mockResolvedValueOnce(createRunResult('What do you want?', 'some-session'));
  ...
  describe('error handling', () => {
    it('should surface Codex execution errors', async () => {
      mockRunCodexText.mockRejectedValueOnce(new Error('boom'));
  ...
  describe('editText with cursor mode', () => {
    it('should handle cursor mode request', async () => {
      mockRunCodexText.mockResolvedValueOnce(
        createRunResult('<insertion>fox</insertion>', 'cursor-session')
      );
      ...
      expect(mockRunCodexText.mock.calls[0][0].prompt).toContain('<editor_cursor path="test.md" line="1">');
    });

    it('should handle inbetween mode request', async () => {
      mockRunCodexText.mockResolvedValueOnce(
        createRunResult('<insertion>## Description\n\nNew section content</insertion>', 'inbetween-session')
      );
      ...
      expect(mockRunCodexText.mock.calls[0][0].prompt).toContain('| #inbetween');
    });
  });
});
