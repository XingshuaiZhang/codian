...
describe('AgentSettings save orchestration', () => {
  ...
  it('renaming saves with filePath undefined, then deletes old file', async () => {
    const existing = createAgent('old-name', '.codex/agents/custom-old.md');
    const renamed = createAgent('new-name', '.codex/agents/custom-old.md');
  ...
  it('non-rename saves original agent and does not delete', async () => {
    const existing = createAgent('same-name', '.codex/agents/custom-name.md');
    const updated = createAgent('same-name', '.codex/agents/custom-name.md');
  ...
  it('shows notice and aborts when loading existing agent fails', async () => {
    const existing = createAgent('existing-agent', '.codex/agents/existing-agent.md');
