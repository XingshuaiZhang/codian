...
describe('utils.ts', () => {
  ...
  describe('getModelsFromEnvironment', () => {
    it('should extract model from CODEX_MODEL', () => {
      const envVars = { CODEX_MODEL: 'gpt-5-codex' };
      const result = getModelsFromEnvironment(envVars);

      expect(result).toHaveLength(1);
      expect(result[0].value).toBe('gpt-5-codex');
      expect(result[0].description).toContain('codex');
    });

    it('should extract models from both supported env variables', () => {
      const envVars = {
      ...
      expect(result.map(m => m.value)).toContain('custom-codex');
      expect(result.map(m => m.value)).toContain('custom-openai');
    });
    ...
    it('should handle model names with slashes (provider/model format)', () => {
      const envVars = { CODEX_MODEL: 'openai/gpt-5-codex' };
      const result = getModelsFromEnvironment(envVars);

      expect(result).toHaveLength(1);
      expect(result[0].value).toBe('openai/gpt-5-codex');
      expect(result[0].label).toBe('gpt-5-codex');
    });

    it('should fallback to full value when slash-split yields empty', () => {
      const envVars = { CODEX_MODEL: 'trailing-slash/' };
      const result = getModelsFromEnvironment(envVars);
    ...
    it('should keep CODEX_MODEL ahead of OPENAI_MODEL', () => {
      const envVars = {
  ...
  describe('getCurrentModelFromEnvironment', () => {
    it('should return CODEX_MODEL if set', () => {
      const envVars = {
        CODEX_MODEL: 'main-model',
        OPENAI_MODEL: 'fallback-model',
      };
    ...
    it('should return OPENAI_MODEL if CODEX_MODEL not set', () => {
      const envVars = {
  ...
  describe('findCodexCLIPath', () => {
    ...
    describe('on Unix/macOS', () => {
      ...
      it('should return first matching Codex CLI path via the Codex compatibility alias', () => {
        jest.spyOn(os, 'homedir').mockReturnValue('/home/test');
        mockExistingFile('/home/test/.local/bin/codex');

        expect(findCodexCLIPath()).toBe('/home/test/.local/bin/codex');
      });

      it('should return null when Codex CLI is not found', () => {
        jest.spyOn(os, 'homedir').mockReturnValue('/home/test');
      ...
      it('should check Codex cli.js paths as fallback on Unix', () => {
        jest.spyOn(os, 'homedir').mockReturnValue('/home/test');
      ...
      it('should resolve Codex CLI from custom PATH', () => {
        ...
        expect(findCodexCLIPath(customPath)).toBe('/custom/bin/codex');
      });
      ...
      it('should expand home directory in custom PATH', () => {
        ...
        expect(findCodexCLIPath(customPath)).toBe('/home/test/bin/codex');
      });
    ...
    describe('on Windows', () => {
      ...
      it('should resolve cli.js from custom PATH npm prefix', () => {
        ...
        expect(findCodexCLIPath(customPath)).toBe(cliJsPath);
      });
  ...
  describe('getPathAccessType', () => {
    ...
    it('should allow vault access to safe ~/.codex/ subdirectories', () => {
      jest.spyOn(os, 'homedir').mockReturnValue('/home/test');
      const realpathSpy = jest.spyOn(fs, 'realpathSync').mockImplementation((p: any) => String(p) as any);
      (fs.realpathSync as any).native = realpathSpy;

      expect(getPathAccessType('/home/test/.codex', [], [], '/vault')).toBe('context');
      expect(getPathAccessType('/home/test/.codex/settings.json', [], [], '/vault')).toBe('vault');
      expect(getPathAccessType('/home/test/.codex/hooks/pre-commit.sh', [], [], '/vault')).toBe('context');
    });

    it('should allow access to ~/.codex/ via tilde expansion', () => {
      jest.spyOn(os, 'homedir').mockReturnValue('/home/test');
      const realpathSpy = jest.spyOn(fs, 'realpathSync').mockImplementation((p: any) => String(p) as any);
      (fs.realpathSync as any).native = realpathSpy;

      expect(getPathAccessType('~/.codex', [], [], '/vault')).toBe('context');
      expect(getPathAccessType('~/.codian/obsidian/sessions/abc.jsonl', [], [], '/vault')).toBe('vault');
    });
  ...
  describe('Windows separator normalization', () => {
    ...
    it('treats ~/.codex paths as vault access after normalization', () => {
      jest.spyOn(os, 'homedir').mockReturnValue('C:\\Users\\test');
      expect(getPathAccessType(
        'C:\\Users\\test\\.codex\\settings.json',
        [],
        [],
        'C:\\Users\\test\\vault'
      )).toBe('vault');
