...
describe('getPathAccessType', () => {
  ...
  it('returns vault for ~/.codex safe subdirectory', () => {
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'settings.json'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'sessions', 'abc.jsonl'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'obsidian', 'runtime-settings.json'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'obsidian', 'commands', 'cmd.md'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'agents', 'agent.md'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.agents', 'skills', 'skill', 'SKILL.md'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'plans', 'plan.md'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'obsidian', 'mcp.json'), [], [], vaultPath)).toBe('vault');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'obsidian', 'settings.json'), [], [], vaultPath)).toBe('vault');
  });

  it('returns context (read-only) for unknown ~/.codex paths', () => {
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'credentials'), [], [], vaultPath)).toBe('context');
    expect(getPathAccessType(path.join(os.homedir(), '.codex', 'secrets.json'), [], [], vaultPath)).toBe('context');
  });

  it('returns context for ~/.codex directory itself', () => {
    expect(getPathAccessType(path.join(os.homedir(), '.codex'), [], [], vaultPath)).toBe('context');
  });
...
describe('findCodexCLIPath', () => {
  ...
  it('returns null when nothing found', () => {
    ...
    const result = findCodexCLIPath('/nonexistent/path');
  ...
  it('resolves from custom path entries', () => {
    ...
    jest.spyOn(fs, 'statSync').mockImplementation(
      p => ({ isFile: () => String(p) === cliPath }) as fs.Stats
    );

    const result = findCodexCLIPath(isWindows ? 'C:\\custom\\bin' : '/custom/bin');
    expect(result).toBe(cliPath);
  });

  it('returns string or null', () => {
    const result = findCodexCLIPath();
    expect(result === null || typeof result === 'string').toBe(true);
  });

  it('finds codex from common paths when no custom path provided', () => {
  ...
  it('falls back to PATH environment when common and npm paths fail', () => {
    const envCliPath = '/env/specific/bin/codex';
    const originalPath = process.env.PATH;
    process.env.PATH = `/env/specific/bin:${originalPath}`;

    jest.spyOn(fs, 'existsSync').mockImplementation(
      p => String(p) === envCliPath
    );
    jest.spyOn(fs, 'statSync').mockImplementation(
      p => ({ isFile: () => String(p) === envCliPath }) as fs.Stats
    );

    try {
      const result = findCodexCLIPath();
      expect(result).toBe(envCliPath);
    } finally {
  ...
  it('returns null for custom path without codex binary on non-Windows', () => {
    ...
    const result = findCodexCLIPath(customDir);
    expect(result).toBeNull();
  });
  ...
  it('handles inaccessible filesystem paths gracefully', () => {
    ...
    const result = findCodexCLIPath('/some/path');
    expect(result).toBeNull();
  });

  it('finds codex via nvm default version when NVM_BIN is not set (Unix)', () => {
    if (isWindows) return;
  ...
  it('finds codex via built-in nvm node alias when NVM_BIN is not set (Unix)', () => {
    if (isWindows) return;
