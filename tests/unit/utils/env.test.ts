...
describe('withProxyAliases', () => {
  it('adds lowercase proxy aliases when only uppercase keys are present', () => {
    expect(withProxyAliases({
  ...
  it('adds uppercase proxy aliases when only lowercase keys are present', () => {
    expect(withProxyAliases({
...
describe('getWslForwardedEnvironment', () => {
  it('omits PATH and preserves custom runtime variables for WSL exec', () => {
    expect(getWslForwardedEnvironment({
...
describe('resolveWindowsWslExecutable', () => {
  ...
  it('prefers an absolute System32 wsl.exe path when present', () => {
    process.env.SystemRoot = 'C:\\Windows';
    delete process.env.windir;
    const candidate = path.win32.join('C:\\Windows', 'System32', 'wsl.exe');

    jest.spyOn(fs, 'existsSync').mockImplementation(p => String(p) === candidate);

    expect(resolveWindowsWslExecutable()).toBe(candidate);
  });

  it('falls back to bare wsl.exe when no absolute candidate exists', () => {
    delete process.env.SystemRoot;
    delete process.env.windir;
    delete process.env.SYSTEMROOT;
    delete process.env.WINDIR;
    ...
    expect(resolveWindowsWslExecutable()).toBe('wsl.exe');
  });

  it('derives wsl.exe from ComSpec when SystemRoot is unavailable', () => {
    delete process.env.SystemRoot;
    delete process.env.windir;
    delete process.env.SYSTEMROOT;
    delete process.env.WINDIR;
    ...
    delete process.env.Path;
    delete process.env.PATH;
    const candidate = path.win32.join('D:\\Windows', 'System32', 'wsl.exe');
  ...
  it('scans Windows drive roots when environment variables are missing', () => {
    delete process.env.SystemRoot;
    delete process.env.windir;
    delete process.env.SYSTEMROOT;
    delete process.env.WINDIR;
...
describe('getCustomModelIds', () => {
  ...
  it('should extract CODEX_MODEL', () => {
    const result = getCustomModelIds({ CODEX_MODEL: 'custom-model' });
    expect(result.size).toBe(1);
    expect(result.has('custom-model')).toBe(true);
  });

  it('should extract OPENAI_MODEL', () => {
    const result = getCustomModelIds({
      OPENAI_MODEL: 'my-openai-model',
    });
    expect(result.size).toBe(1);
    expect(result.has('my-openai-model')).toBe(true);
  });
...
describe('getModelsFromEnvironment', () => {
  ...
  it('returns model for CODEX_MODEL', () => {
    const result = getModelsFromEnvironment({ CODEX_MODEL: 'custom-model-v1' });
  ...
  it('formats label from hyphenated model name', () => {
    const result = getModelsFromEnvironment({ CODEX_MODEL: 'gpt-5-codex' });
    expect(result[0].label).toBe('Gpt 5 Codex');
  });

  it('formats label from slash-separated model name', () => {
    const result = getModelsFromEnvironment({ CODEX_MODEL: 'org/custom-model' });
    expect(result[0].label).toBe('custom-model');
  });

  it('returns models for both supported env vars', () => {
    const result = getModelsFromEnvironment({
      CODEX_MODEL: 'my-codex-model',
      OPENAI_MODEL: 'my-openai-model',
    });
    expect(result).toHaveLength(2);
    expect(result.map(m => m.value)).toContain('my-codex-model');
    expect(result.map(m => m.value)).toContain('my-openai-model');
  });
  ...
  it('prefers CODEX_MODEL ordering before OPENAI_MODEL', () => {
    const result = getModelsFromEnvironment({
      CODEX_MODEL: 'main-model',
      OPENAI_MODEL: 'fallback-model',
    });
    expect(result[0].value).toBe('main-model');
    expect(result[1].value).toBe('fallback-model');
  });
...
describe('getCurrentModelFromEnvironment', () => {
  ...
  it('returns CODEX_MODEL when set', () => {
    expect(getCurrentModelFromEnvironment({
      CODEX_MODEL: 'custom-model',
    })).toBe('custom-model');
  });

  it('prefers CODEX_MODEL over OPENAI_MODEL', () => {
    expect(getCurrentModelFromEnvironment({
      CODEX_MODEL: 'main-model',
      OPENAI_MODEL: 'fallback-model',
    })).toBe('main-model');
